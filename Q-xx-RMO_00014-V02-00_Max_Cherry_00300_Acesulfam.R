...
lapply(dt$raw, nrow)
dt$raw <- lapply(dt$raw, \( x ) x[ seq(1, nrow(x), 5) , ])
...
# PLS para ####
dt$para.pls$wl <- 195:280
dt$model.raw$data$Probe == dt$para$substance[dt$para$i]
dt$para.pls$wlr <- wlr_function(dt$para.pls$wl, dt$para.pls$wl, 5); nrow(dt$para.pls$wlr)
dt$para.pls$wlm <- wlr_function_multi(dt$para.pls$wl, dt$para.pls$wl, 5); nrow(dt$para.pls$wlm)
...
plot(dt$mop$pred.lin
     ...
     , ylim = dt$para$SOLL[ dt$para$i] * c(75, 115) / 100, axes = T
